...
describe('HeroesService', () => {
  ...
  it('debería devolver la cantidad total de héroes activos', () => {
    ...
    const todos = service['allHeroes']().filter(h => h.activo);
  ...
  it('debería aplicar correctamente el filtro', () => {
    ...
    const total = service.totalHeroes();
    const coincide = service['allHeroes']().filter(h =>
      h.activo &&
  ...
  it('debería editar un héroe existente', () => {
    const primerHeroe = service['allHeroes']()[0];
  ...
  it('debería eliminar lógicamente un héroe', () => {
    const primerHeroe = service['allHeroes']().find(h => h.activo)!;
    service.eliminarLogico(primerHeroe.id);
    const total = service.totalHeroes();
    expect(service['allHeroes']().find(h => h.id === primerHeroe.id)?.activo).toBeFalse();
    expect(total).toBeLessThan(service['allHeroes']().length);
  });
